...
function callTakeHostage()
	...
	if not canTakeHostage then 
		drawNativeNotification("Du brauchst eine geladene Waffe um eine Geisel zunehmen!")
	end
	...
	if not takeHostage.InProgress and canTakeHostage then			
		...
		if closestPlayer then
			...
			if targetSrc ~= -1 then
				...
				drawNativeNotification("~r~Keiner in der nähe um ihn als Geisel zunehmen")
			end
		else
			drawNativeNotification("~r~Keiner in der nähe um ihn als Geisel zunehmen")
		end
...
Citizen.CreateThread(function()
	while true do 
		if takeHostage.type == "agressor" then
			...
			drawNativeText("Drücke ~g[G] um Freizulassen & Drücke ~r[H] um zuschießen ")
